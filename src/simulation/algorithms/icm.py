...
class ICM(object):
    ...
    def compute_irs(self, samples: Dict, step: int = 0) -> th.Tensor:
        ...
        samples['next_obs'] = samples['obs'][1:]
        
        
        obs = th.from_numpy(samples["obs"])
        actions = th.from_numpy(samples["actions"])
        next_obs_tensor = th.from_numpy(samples["next_obs"]).to(self._device)
        ...
        with th.no_grad():
            for i in range(num_envs):
                ...
                dist =th.linalg.norm(encoded_next_obs-pred_next_obs[:-1], ord=2,dim=1)
                intrinsic_rewards[:-1,i] = dist.cpu()


        self.update(samples)

        return (intrinsic_rewards * beta_t).numpy()
    ...
    def update(self, samples) -> None:
        ...
        next_obs_tensor = th.from_numpy(samples["next_obs"]).view((num_envs * num_steps-1, *self._obs_shape)).to(self._device)
        actions_tensor = th.from_numpy(samples["actions"]).view((num_envs * num_steps, self._action_dim)).to(self._device)
        
        
        dataset = TensorDataset(obs_tensor[:-1], actions_tensor[:-1], next_obs_tensor)
