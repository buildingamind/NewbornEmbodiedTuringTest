# This file contains the architecture of ResNet-18 (2 blocks)
...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x = self.layer2(x)
        #print("En: After layer2 size -", x.shape)


        x = self.avgpool(x)
        #print("En: After avgpool size -", x.shape)
        x = torch.flatten(x, 1)
        #print("En: After flattening size -", x.shape)
        return x
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, block, layers, latent_dim, input_height, first_conv=False, maxpool1=False):
        ...
        self.upscale3 = Interpolate(size=input_height // 1)
...
def resnet18_encoder_2b(first_conv, maxpool1):
    return ResNetEncoder(EncoderBlock, [2, 2, 2, 2], first_conv, maxpool1)


def resnet18_decoder_2b(latent_dim, input_height, first_conv, maxpool1):
