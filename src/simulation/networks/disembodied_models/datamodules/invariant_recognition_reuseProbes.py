...
# resuse and test the trained probes on mutiple test datasets.
...
class InvariantRecognition(Dataset):
    def __init__(
        ...
        self.dataframe = self.create_dataframe(data_dir, self.training, self.drop_samples)
        self.transform = transform
        
        print("the data_dir is : ", data_dir)
        #print("samples dropped are: ", self.drop_samples)

    def create_dataframe(self, data_dir: str, training: bool, drop_samples: int) -> pd.DataFrame:
        dfs = []
        ...
        for subdir in subdirs:
            ...
            # Only include images of the specified viewpoints.
            if viewpoint in self.viewpoints:
                ...
                if self.training == True:
                    ...
                    df.drop(df.tail(drop_samples).index, inplace = True) # set how many values 
                    #print(df)
                else:
                    #print("False")
                    
                    df.drop(df.tail(drop_samples).index, inplace = True)
                    #print(df)
                dfs.append(df)
...
class InvariantRecognitionDataModule_RP(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        super().__init__()
        self.train_data_dir = train_data_dir            # TRAINING SET
        self.test_data_dir = test_data_dir  # TEST SET
        self.identifier = identifier
        self.num_folds = num_folds
        self.val_fold = val_fold #fold_idx
        self.num_workers = num_workers
        self.batch_size = batch_size
        self.shuffle = True # added on-top
        ...
        self.drop_test_samples = 0 # only when the test samples are 1k/viewpoints
    ...
    def setup(self, stage: Optional[str] = None) -> None: # lighteningDataModule prebuilt function
        ...
        if stage == "fit" or stage is None:
            ...
            train_viewpoints, val_viewpoints = self.viewpoint_splits # returns two lists.

            # TRAIN DATASET DIRECTORY IS DIFFERENT
            self.dataset_train = self.dataset_cls(
                self.train_data_dir, 
                self.drop_train_samples, 
                viewpoints=train_viewpoints, 
                transform=train_transforms, 
                training=True
                ) # extra argument to know when training view is passed

            # VAL DATASET DIRECTORY IS SAME AS TRAIN DATASET
        ...
        # TEST DATASET DIRECTORY IS DIFFERENT FROM TRAIN AND VAL
        if stage == "test" or stage is None:
            test_transforms = self.default_transforms if self.test_transforms is None else self.test_transforms
            val_transforms = self.default_transforms if self.val_transforms is None else self.val_transforms

            # the second parameter here is 0 because we are not dropping any samples from test set.
            # if test set has more than 1k samples/viewpoints then you should drop.
            self.dataset_test = self.dataset_cls(
                self.test_data_dir, 
                self.drop_test_samples, 
                viewpoints = val_viewpoints,
