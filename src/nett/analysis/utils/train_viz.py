...
def train_viz(data_loc, results_wd):
    ...
    Parameters:
        data_loc (str): Path to the merged data CSV file.
    ...
    train_data["correct_steps"] = np.where(
        condition, train_data["left_steps"], train_data["right_steps"]
    )
    train_data["incorrect_steps"] = np.where(
        condition, train_data["right_steps"], train_data["left_steps"]
    )
    train_data["percent_correct"] = train_data["correct_steps"] / (
        train_data["correct_steps"] + train_data["incorrect_steps"]
    )
    ...
    summary = (
        train_data.groupby(["imprint.cond", "agent", "episode_block"])[
            "percent_correct"
        ]
        .agg(avgs="mean", sd="std", count="size")
        .reset_index()
    )
    ...
    for cond in summary["imprint.cond"].unique():
        ...
        plt.ylim(0, 1)
        plt.yticks(
            np.arange(0, 1.1, 0.1), ["{:.0%}".format(x) for x in np.arange(0, 1.1, 0.1)]
        )
