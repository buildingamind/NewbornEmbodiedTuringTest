...
def make_bar_charts(data, dots, y_col, error_min_col, error_max_col,
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(x_categories, rotation=0, ha='center', fontsize=7.5, fontweight='bold')
    ...
    if not chick_data_filtered.empty:
        ...
        for xi, yi, yerr, i in zip(chick_x_pos, chick_data_filtered['avg'],
            ...
            if x_categories[i] == 'Rest':
                shift = 1
            rect = plt.Rectangle((xi + shift - 0.35, yi - yerr), 0.7, 2*yerr, color=CHICK_RED, alpha=0.2)
            ax.add_patch(rect)
