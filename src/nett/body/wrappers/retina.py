...
class Retina(gym.ObservationWrapper):
    ...
    def __init__(
        ...
        try:
            ...
            self.retina = ArtificialRetina(P=width, fovea_center=(height//2, width//2), fovea_radius=height//4)
    ...
    def observation(self, obs):
        ...
        if len(obs) > 0:
            ...
            out = self.retina.process(self.retina, image=prev, next_image=current)

        else:
            obs = np.transpose(obs, (1, 2, 0))

            out = self.retina.process(self.retina, image=obs)
...
class ArtificialRetina:
    ...
    def process(self,image, next_image=None):
        ...
        if self.foveation_type == 'dynamic':
            ...
            fovea_x, fovea_y = self.dynamic_fovea(prev_frame=preprocessed_image, current_frame=next_image, grid_size=self.dynamic_foveation_grid_size)
    ...
    def radial_pixel_distortion(self, image, max_distortion=10, distortion_intensity=1.0):
