...
def fast(config):
	...
	conditions = config_text["Run"]["conditions"]
	...
	devices = config_text["Run"]["devices"]
	...
	# free_device_memory: dict[int, int] = {device: memory_status["free"] for device, memory_status in get_memory_status(devices).items()}

	# free_devices: list[int] = devices.copy()
	# job_memory = config_text["Run"]["job_memory"] * 1024**3

	# for condition in conditions:
	# 	for device in free_devices:
	# 		if free_device_memory[device] < job_memory:
	# 			free_devices.remove(device)
	# 		else:
	# 			free_device_memory[device] -= job_memory
	# 			# schedule the job for brain_id 1 and condition
	# 			break
	...
	num_envs = config_text["Run"]["num_brains"] * len(conditions)
		...
		while True:
			try:
				def make_env(i):
					...
					brain_id = 1
					base_path = Path(config_text["Run"]["output_dir"]) / condition /f"brain_{brain_id}"
					return lambda: Environment(mode, port+i, executable_path, reward, base_path, condition, steps_per_episode)
				...
				if mode == "train":
					train(envs, algorithm, Path(config_text["Run"]["output_dir"]), iterations[mode], device, reward, config_text)
				else:
					test(envs, algorithm, Path(config_text["Run"]["output_dir"]), iterations[mode], device)
...
# def get_memory_status(devices: list[int]) -> dict[int, dict[str, int]]:
# 		nvmlInit()
